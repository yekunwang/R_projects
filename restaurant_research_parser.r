...
## Author: Yekun
## Date: 8/22/16
## Description: execution steps for RR LTO data
...
fileLoader <- function(file,endRow,endCol){
  ...
  raw <- as.data.frame(XLConnect::readWorksheet(loadWorkbook(file),sheet=1,startRow=4,startCol=1,endRow=endRow,endCol=endCol), 
...
# "label" (name of this promotion) and "description" (details about this promotion).
  ...
  if(is.na(string)){
    ...
    if(!str_detect(string,"\\(")){
      ...
      tbl <- data.frame(row.names=NULL,
                        label=items,
                        description=rep("",n=length(items)),stringsAsFactors = FALSE)
    }else{
      # With parenthesis
      if(sum(str_count("\\("))!=sum(str_count("\\)"))){
        stop("Mismatched parenthesis!")
      }else{
        # parse out () information as description
        descriptions <- gsub("[\\(\\)]", "", regmatches(string, gregexpr("\\(.*?\\)", string))[[1]],perl = TRUE) %>% str_trim(.,side="both")
        ...
        # split into with and without description
        ...
        tbl <- data.frame(row.names=NULL,
          ...
          rbind(data.frame(label=nofill,description=rep("",length(nofill))))
...
brandAggregator <- function(raw,fileName){  
  ...
  agg_df$source <- "Restaurant Research"
  agg_df$score <- ""
  ...
  agg_df$startDate <- format(as.Date(str_replace_all(agg_df$startDate,"-"," 01 "),"%b %d %y"),"%Y-%m-%d")
  agg_df$endDate <- format(ymd(as.Date(str_replace_all(agg_df$endDate,"-"," 01 "),"%b %d %y"))+months(1)-days(1),"%Y-%m-%d")
  
  # Re-order columns
  agg_df %<>% select(externalId,eventType,label,description,startDate,endDate,brand,source,score)
...
inputFile <- "/Users/yekun/Desktop/restaurant_research/Monthly_Promotions_Database_MasterSheet.xls"
endRow  <- 67
endCol <- 139
outputFile<- "~/Desktop/testing.xls"
...
newFile <- fileLoader(file=as.character(inputFile),endRow=as.numeric(endRow),endCol=as.numeric(endCol))
# newFile <- fileLoader(file=as.character("~/qanalysis/Users/Yekun/restaurant_research/Monthly_Promotions_Database_MasterSheet.xls"),endRow=as.numeric(67),endCol=as.numeric(125))
# Example source file: "~/Desktop/restaurant_reviews/Monthly_Promotions_Database_MasterSheet copy.xls"

# 5. Run brandTester on newFile

cat("\n")
cat("Testing individual brand...")
cat("\n")
cat("Please make sure every brand passes the test!")
cat("\n")

brandTester(newFile)
# Errored on one brand

# 6. Correct format in the original file
# 7. Run brand aggregator

cat("\n")
cat("Generating aggregated excel sheet...")
cat("\n")
brandAggregator(newFile,fileName = as.character(outputFile))
brandAggregator(newFile,fileName = as.character("~/qanalysis/Users/Yekun/restaurant_research/example.csv"))
# Example sink file: "~/Desktop/testing.xls"

cat("\n")

# Rscript run_parser.r "~/Desktop/restaurant_reviews/Monthly_Promotions_Database_MasterSheet copy.xls" 69 120 "~/Desktop/testing.xls"